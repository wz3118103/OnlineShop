...
public class UserAwardMapDaoTest extends BaseTest {
    ...
    public void testBQueryUserAwardMapList() throws Exception {
        ...
        assertEquals("测试奖品", userAwardMap.getAward().getAwardName());
    }
    ...
    public void testCUpdateUserAwardMap() throws Exception {
        ...
        customer.setName("测试");
        userAwardMap.setUser(customer);
        List<UserAwardMap> userAwardMapList = userAwardMapDao.queryUserAwardMapList(userAwardMap, 0, 2);
        for (UserAwardMap awardMap : userAwardMapList) {
            System.out.println(awardMap.getUserAwardId() + ", " + awardMap.getUsedStatus());
        }
//        assertTrue("Error, 积分不一致！", 0 == userAwardMapList.get(1).getUsedStatus());
//        userAwardMapList.get(1).setUsedStatus(1);
